...
def cumulative(outdir, rows):
    chosen=["P0","P1","P3","P7","P8","P9"]; x0,y0,x1,y1=80,90,950,525
...
def distributions(outdir, raw):
    ...
    lo=min(allv);hi=max(allv);bins=32;step=(hi-lo)/bins;x0,y0,x1,y1=80,90,950,525
    density={}; ymax=0
    ...
    for pid in ("P0","P8"):
        ...
        b+=poly(pts,COLORS[pid],3)+text(scale(mu,lo,hi,x0,x1),105,f"{pid} mean {mu:,.0f}",12,"middle",COLORS[pid],"bold")
...
def risk_return(outdir, summary):
    x0,y0,x1,y1=90,90,950,520;xs=[float(r["std_objective"]) for r in summary];ys=[float(r["worst_10pct_objective"]) for r in summary];xmin=min(xs)*.9;xmax=max(xs)*1.05;ymin=min(ys)*.92;ymax=max(ys)*1.05
    ...
    for r in summary:
        ...
        b+=circle(x,y,size,COLORS[p],opacity=.85)+text(x+12,y-10,p,12,"start",COLORS[p],"bold")
    b+=text((x0+x1)/2,557,"Standard deviation (lower is safer)",13)+text(22,(y0+y1)/2,"Worst 10% mean (higher is better)",13,rotate=-90)
...
def projects(outdir, internal, external):
    ...
    x0,y0,x1,y1=90,90,950,520;xs=[x[2] for x in items];ys=[x[3] for x in items];xt=[(v,scale(v,0,1,x0,x1)) for v in (0,.2,.4,.6,.8,1)];yt=[(v,scale(v,0,max(ys)*1.08,y1,y0)) for v in range(0,1001,200)]
    ...
    b+=circle(720,555,7,"#4E79A7",opacity=.7)+text(733,559,"Internal",11,"start")+circle(810,555,7,"#F28E2B",opacity=.7)+text(823,559,"External",11,"start")
...
def main(data,out):
    ...
    audit={"generator":"dependency-free SVG","figure_count":7,"figures":{}}
    ...
    print("Generated 7 SVG dissertation figures in",out)
